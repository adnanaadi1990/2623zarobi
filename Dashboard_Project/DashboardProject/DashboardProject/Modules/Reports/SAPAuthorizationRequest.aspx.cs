...
{
    public partial class SAPAuthorizationRequest : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                ddlTCode.Items.Insert(0, new ListItem("------Select------", ""));
            }
        }
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ds.Clear();
                cmd.CommandText = "";
                cmd.CommandText = @"SP_TCodeAuthorizationForm";
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Connection = conn;
                cmd.Parameters.AddWithValue("@FormIDFrom", txtFormIDfrom.Text);
                cmd.Parameters.AddWithValue("@FormIDto", txtFormIDto.Text);
                cmd.Parameters.AddWithValue("@Module", ddlDBModules.SelectedValue.ToString().Replace("0", ""));
                cmd.Parameters.AddWithValue("@TCode", ddlTCode.SelectedValue.ToString());
                cmd.Parameters.AddWithValue("@Report", TxtReportTCode.Text);
        ...
        protected void bindTCode()
        {
            ...
            strQuery = @"SELECT distinct  LTRIM(RTRIM(TCode)) as TCode ,TCode + ' ' + Description as Description from tbl_TCode";
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                conn.Open();
                ds.Clear();
                adp.Fill(ds, "tbl_TCode");

                ddlTCode.DataTextField = ds.Tables["tbl_TCode"].Columns["Description"].ToString(); // text field name of table dispalyed in dropdown
                ddlTCode.DataValueField = ds.Tables["tbl_TCode"].Columns["TCode"].ToString().Trim(); ;             // to retrive specific  textfield name 
                ddlTCode.DataSource = ds.Tables["tbl_TCode"];      //assigning datasource to the dropdownlist
